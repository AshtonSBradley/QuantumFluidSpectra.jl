...
function energydecomp(psi::Psi{2})
    ...
    et = @. abs2(wx) + abs2(wy)
    et *= 0.5
    ei = @. abs2(wxi) + abs2(wyi)
    ei *= 0.5
    ec = @. abs2(wxc) + abs2(wyc)
    ec *= 0.5
    return et, ei, ec
end
...
function energydecomp(psi::Psi{3})
    ...
    wxi, wyi, wzi = Wi
    wxc, wyc, wzc = Wc
    et = @. abs2(wx) + abs2(wy) + abs2(wz)
    et *= 0.5
    ei = @. abs2(wxi) + abs2(wyi) + abs2(wzi)
    ei *= 0.5
    ec = @. abs2(wxc) + abs2(wyc) + abs2(wzc)
    ec *= 0.5
    return et, ei, ec
end
...
cross_correlate(psi1::Psi{D}, psi2::Psi{D}) where {D} =
...
"""
function gpe_particle_transfer(
    k,
    psi::Psi;
    g = 1.0,
    V = nothing,
